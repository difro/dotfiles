...
현재 dotfiles 구조에서는 이 flake가 `base` 저장소 안의 `home-manager/pkgs/claude-code` 경로에 있으므로, 원격 설치 시 `dir=home-manager/pkgs/claude-code`를 같이 지정하면 됩니다.

예시:

```bash
nix profile install 'git+ssh://git@oss.navercorp.com/ORG/BASE_REPO.git?ref=main&dir=home-manager/pkgs/claude-code#claude-code'
```

기본 패키지는 `default`도 내보내므로 아래처럼 써도 됩니다.

```bash
nix profile install 'git+ssh://git@oss.navercorp.com/ORG/BASE_REPO.git?ref=main&dir=home-manager/pkgs/claude-code'
```

설치 대상을 특정 커밋으로 고정하려면 `rev`를 같이 넣으면 됩니다.

```bash
nix profile install 'git+ssh://git@oss.navercorp.com/ORG/BASE_REPO.git?ref=main&rev=COMMIT_SHA&dir=home-manager/pkgs/claude-code#claude-code'
```
...
현재 dotfiles 구조에서는 상위 Home Manager flake가 바로 한 단계 위 디렉터리에 있으므로, 가장 단순한 방법은 `path:./pkgs/claude-code` input을 추가하고 overlay를 통해 `pkgs.claude-code`를 쓰는 것입니다.
...
```nix
{
  inputs = {
    ...
    claude-code.url = "path:./pkgs/claude-code";
  };
...
```nix
inputs.claude-code.url = "path:./pkgs/claude-code";
overlays = [ claude-code.overlays.default ];
```
...
```nix
nix profile install "$HOME/.dotfiles/base/home-manager/pkgs/claude-code#claude-code"
```

GHES 저장소를 input으로 잡는다면 `dir=`를 같이 지정합니다.

```nix
inputs.claude-code.url = "git+ssh://git@oss.navercorp.com/ORG/BASE_REPO.git?ref=main&dir=home-manager/pkgs/claude-code";
```
